...
# sensible defaults to avoid empty input errors on first run
num_value.set("10")
ev_value.set("4")
level_value.set("1")
make_value.set("Toyota")
model_value.set("Corolla")
color_value.set("Blue")
reg_value.set("ABC123")
    
tfield = tk.Text(root, width=70, height=15, bg='white', highlightthickness=1, highlightbackground='grey')
# make the output box visibly present
try:
    tfield.insert(tk.INSERT, "Output will appear here...\n")
except Exception:
    pass
...
class ParkingLot:
    ...
    def status(self):
        ...
        try:
            tfield.see(tk.END)
            tfield.focus_set()
            messagebox.showinfo("Status", "Current lot status printed in the box below.")
        except Exception:
            pass
    ...
    def chargeStatus(self):
        ...
        try:
            tfield.see(tk.END)
            tfield.focus_set()
            messagebox.showinfo("EV Charge Status", "Charge status printed in the box below.")
        except Exception:
            pass
    ...
    def makeLot(self):
        if not num_value.get() or not ev_value.get() or not level_value.get():
            tfield.insert(tk.INSERT, "Please enter Regular, EV, and Level numbers before creating the lot.\n")
            return
        try:
            ...
            tfield.insert(tk.INSERT, output)
            try:
                messagebox.showinfo("Parking Lot", output.strip())
            except Exception:
                pass
        except ValueError:
            tfield.insert(tk.INSERT, "Please enter valid integers for Regular, EV, and Level.\n")

    def parkCar(self):  
        try:
            if not reg_value.get() or not make_value.get() or not model_value.get() or not color_value.get():
                tfield.insert(tk.INSERT, "Please fill Make, Model, Color, and Registration before parking.\n")
                return
            ...
            if res == -1:
                ...
                tfield.insert(tk.INSERT, output)
                try:
                    messagebox.showinfo("Parking", output.strip())
                except Exception:
                    pass
        except Exception as e:
            tfield.insert(tk.INSERT, f"Error parking vehicle: {e}\n")

    def removeCar(self):
        try:
            # Try regular slot first (slot 1)
            status = self.leave(1, 0)  # slot 1, not EV
            if status:
                output = 'Slot number 1 is free\n'
                tfield.insert(tk.INSERT, output)
                try:
                    messagebox.showinfo("Remove Car", output.strip())
                except Exception:
                    pass
            else:
                # Try EV slot (slot 1 in EV area)
                status = self.leave(1, 1)  # slot 1, is EV
                if status:
                    output = 'Slot number 1 (EV) is free\n'
                    tfield.insert(tk.INSERT, output)
                    try:
                        messagebox.showinfo("Remove Car", output.strip())
                    except Exception:
                    ...
                    tfield.insert(tk.INSERT, "Unable to remove a car from slot 1\n")
                    try:
                        messagebox.showwarning("Remove Car", "Unable to remove a car from slot 1")
                    except Exception:
                        pass
        except Exception as e:
...
def main():
    ...
    parkMakeBtn = tk.Button(root, command = parkinglot.makeLot, text = "Create Parking Lot", font="Arial 12", bg='lightblue', fg='black', activebackground="#008080", padx=5, pady=5 )
    ...
    parkBtn = tk.Button(root, command = parkinglot.parkCar, text = "Park Car", font="Arial 11", bg='lightblue', fg='black', activebackground="#008080", padx=5, pady=5 )
    ...
    removeBtn = tk.Button(root, command = parkinglot.removeCar, text = "Remove Car", font="Arial 11", bg='lightblue', fg='black', activebackground="#008080", padx=5, pady=5 )
    removeBtn.grid(column=0, row=11,  padx = 4, pady = 4)

    spacer1 = tk.Label(root, text="")
    spacer1.grid(row=12, column=0)

    slotRegBtn = tk.Button(root, command = parkinglot.slotNumByReg, text = "Get Slot ID by Registration #", font="Arial 11", bg='lightblue', fg='black', activebackground="#008080", padx=5, pady=5 )
    ...
    slotColorBtn = tk.Button(root, command = parkinglot.slotNumByColor, text = "Get Slot ID by Color", font="Arial 11", bg='lightblue', fg='black', activebackground="#008080", padx=5, pady=5 )
    ...
    regColorBtn = tk.Button(root, command = parkinglot.regNumByColor, text = "Get Registration # by Color", font="Arial 11", bg='lightblue', fg='black', activebackground="#008080", padx=5, pady=5 )
    ...
    chargeStatusBtn = tk.Button(root, command = parkinglot.chargeStatus, text = "EV Charge Status", font="Arial 11", bg='lightblue', fg='black', activebackground="#008080", padx=5, pady=5 )
    chargeStatusBtn.grid(column=2, row=14,  padx = 4, pady = 4)

    statusBtn = tk.Button(root, command = parkinglot.status, text = "Current Lot Status", font="Arial 11", bg='#9AFF9A', fg='black', activebackground="#66CD66", padx=5, pady=5 )
    statusBtn.grid(column=0, row=15,  padx = 4, pady = 4)

    tfield.grid(column=0, row=16, padx = 10, pady = 10, columnspan = 4, sticky="nsew")
    try:
        tfield.insert(tk.END, "Output will appear here...\n")
    except Exception:
        pass

    # ensure the bottom row expands so the text box becomes visible
    try:
        for c in range(4):
